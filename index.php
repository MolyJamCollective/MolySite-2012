<?php
    include_once('./templates/globals.php');
    
    $pageTitle = 'What Would Molydeux?';
    $pageHeader = '';
    $pageStyles = array();
    $activeTab = '1';

    $pageScripts = array();
    $PageScriptsRaw = '';

    include_once('./templates/header.php');
?>
<div class="row">
    <div class="span10 offset3"> <img class="auto-size" src="./img/title.png" alt="What Would Molydeux?"/> </div>
  </div>
  <div class="row">
    <div class="span10 offset3"> <img class="auto-size" src="./img/moly.png" /> </div>
  </div>
  <div class="row">
    <div class="span10 offset3">
      <div style="width: auto; height: auto; background-color:#56D34C; padding:0 12px; text-align: center;">
        <h2 class="allcaps cream"> An international 48-hour game jam </h2>
      </div>
    </div>
  </div>
  <div class="row">
    <div class="span10 offset3">
      <div style="width: auto; height: auto; background-color:#40B239; padding:0 12px; text-align: center;">
        <h2 class="allcaps cream"> exploring the works of <a href="http://www.twitter.com/petermolydeux">@petermolydeux</a> </h2>
      </div>
    </div>
  </div>
  <div class="row">
    <div class="span10 offset3">
      <div style="width: auto; height: auto; background-color:#2C8C2E; padding:0 12px; text-align: center;">
        <h2 class="allcaps cream"> March 30th&mdash;April 1st, 2012 </h2>
      </div>
    </div>
  </div>
  <div class="row">
    <div class="span10 offset3">
      <div style="width: auto; height: auto; background-color:#006600; padding:0 12px; text-align: center;">
        <h2 class="allcaps cream"><div id="countbox"></div></h2>
      </div>
    </div>
  </div>
  <div class="row">
    <div class="span10 offset3">
      <div style="width: auto; height: auto; background-color:#1f2325; padding:10px 12px; margin-bottom: 10px;">
        <h3>For the latest updates, visit our <a href="http://whatwouldmolydeux.com/news.php">news page</a>, follow <a href="http://www.twitter.com/molyjam">@molyjam</a> on Twitter, or join us for some chat on freenode: <a href="http://webchat.freenode.net/?channels=#molyjam">#MolyJam</a>. Also there is a <a href="http://batchgeo.com/map/090d1bde6ce8cdb9cc0e129ff0ecd829">map</a>.</h3>
        <h3 class="green"><a href="#locations">Sign up on Facebook now</a></h3>
      </div>
    </div>
  </div>
  <div class="row" id="locations">
    <div class="span5 offset3" id="sf">
      <h3><a href="https://www.facebook.com/events/343017505734553/">San Francisco</a></h3>
      <dl class="dl-horizontal">
        <dt>Anna Kipnis</dt>
        <dd><a href="http://www.twitter.com/doubleanna">@doubleanna</a></dd>
        <dt>Patrick Klepek</dt>
        <dd><a href="http://www.twitter.com/patrickklepek">@patrickklepek</a></dd>
        <dt>Chris Remo</dt>
        <dd><a href="http://www.twitter.com/chrisremo">@chrisremo</a></dd>
        <dt></dt>
        <dd><a href="mailto:molyjamsf@gmail.com">Email the team</a></dd>
      </dl>
    </div>
    <div class="span4 offset2" id="melbourne">
      <h3><a href="https://www.facebook.com/events/249349158493327/">Melbourne, Australia</a></h3>
      <dl class="dl-horizontal">
        <dt>Andrew Brophy</dt>
        <dd><a href="http://www.twitter.com/andrewbrophy">@andrewbrophy</a></dd>
        <dt>Harry Lee</dt>
        <dd><a href="https://twitter.com/leehsl">@leehsl</a></dd>
        <dt></dt>
        <dd><a href="mailto:molyjamaus@gmail.com">Email the team</a></dd>
      </dl>
    </div>
  </div>
  <div class="row">
	<div class="span5 offset3" id="oakland">
      <h3><a href="http://www.facebook.com/events/266654650085748/">Oakland / East Bay</a></h3>
      <dl class="dl-horizontal">
        <dt>Brandon Sheffield</dt>
        <dd><a href="http://www.twitter.com/necrosofty">@necrosofty</a></dd>
        <dd><a href="mailto:molyjamsf@gmail.com">Email the team</a></dd>
      </dl>
    </div>        
    <div class="span4 offset2" id="brigton">
      <h3><a href="https://www.facebook.com/events/325788600811520/">Brighton, UK</a></h3>
      <dl class="dl-horizontal">
        <dt>Edd Parris</dt>
        <dd><a href="http://www.twitter.com/empika">@empika</a></dd>
        <dt></dt>
        <dd><a href="mailto:molyjambton@gmail.com">Email the team</a></dd>
      </dl>
    </div>
  </div>
  <div class="row">
	<div class="span5 offset3" id="nyc">
      <h3><a href="https://www.facebook.com/events/187720164675020/">NYC</a></h3>
      <dl class="dl-horizontal">
        <dt>Ben Johnson</dt>
        <dd><a href="http://www.twitter.com/GameDesignerBen">@GameDesignerBen</a></dd>
        <dt></dt>
        <dd><a href="mailto: molyjamny@gmail.com">Email the team</a></dd>
      </dl>
    </div>    
    <div class="span4 offset2" id="london">
      <h3><a href="https://www.facebook.com/events/273233436088258/">London, UK</a></h3>
      <dl class="dl-horizontal">
        <dt>William Robinson</dt>
        <dd><a href="http://www.twitter.com/MetzoPaino">@MetzoPaino</a></dd>
        <dd><a href="mailto:molyjamldn@gmail.com">Email the team</a></dd>
      </dl>
    </div>
  </div>
  <div class="row">
	<div class="span5 offset3" id="seattle">
      <h3><a href="https://www.facebook.com/events/259071207514213">Seattle</a></h3>
      <dl class="dl-horizontal">
        <dt>Collin Moore</dt>
        <dd><a href="http://www.twitter.com/shinyidol">@shinyidol</a></dd>
        <dt></dt>
        <dd><a href="mailto:molyjamseattle@gmail.com">Email the team</a></dd>
      </dl>
    </div>    
    <div class="span4 offset2" id="montreal">
      <h3><a href="https://www.facebook.com/events/274617842613476/">Montreal, Canada</a></h3>
      <dl class="dl-horizontal">
        <dt>Renaud B&eacute;dard</dt>
        <dd><a href="http://www.twitter.com/renaudbedard">@renaudbedard</a></dd>
        <dd><a href="mailto:molyjammtl@gmail.com">Email the team</a></dd>
      </dl>
    </div>
  </div>
  <div class="row">
	<div class="span5 offset3" id="boston">
      <h3><a href="https://www.facebook.com/events/411836675509369/">Boston</a></h3>
      <dl class="dl-horizontal">
        <dt>Dean Tate</dt>
        <dd><a href="http://www.twitter.com/IamDeanTate">@IamDeanTate</a>
        <dd><a href="mailto:molyjamboston@gmail.com">Email the team</a></dd>
      </dl>
    </div>    
    <div class="span4 offset2" id="sydney">
      <h3><a href="http://molyjamsydney-eorg.eventbrite.com.au/?ebtv=C">Sydney, Australia</a></h3>
      <dl class="dl-horizontal">
        <dt>Adrian Vergara</dt>
        <dd><a href="https://twitter.com/LevelUpAdrian">@LevelUpAdrian</a></dd>
        <dd><a href="mailto:molyjamsydney@gmail.com">Email the team</a></dd>
      </dl>
    </div>
  </div>
  <div class="row">
	<div class="span5 offset3" id="la">
      <h3><a href="https://www.facebook.com/events/321794224546563/">Los Angeles</a></h3>
      <dl class="dl-horizontal">
        <dt>Holden Link</dt>
        <dd><a href="http://www.twitter.com/HoldenLink">@HoldenLink</a>
        <dd><a href="mailto:molyjamla@gmail.com">Email the team</a></dd>
      </dl>
    </div>    
    <div class="span4 offset2" id="munich">
      <h3><a href="https://www.facebook.com/events/134077380052626/">Munich, Germany</a></h3>
      <dl class="dl-horizontal">
        <dt>Mustafa K. Isik</dt>
        <dd><a href="https://twitter.com/codesurgeon">@codesurgeon</a></dd>
        <dd><a href="mailto:molyjammunich@codesurgeon.com">Email the team</a></dd>
      </dl>
    </div>
  </div>
  <div class="row">
	<div class="span5 offset3" id="orlando">
      <h3><a href="https://www.facebook.com/events/305335546201015/">Orlando</a></h3>
      <dl class="dl-horizontal">
        <dt>Zane Wolfgang Pickett</dt>
        <dd><a href="https://twitter.com/SirW0lfgang">@SirW0lfgang</a>
        <dd><a href="mailto:molyjamorlando@gmail.com">Email the team</a></dd>
      </dl>
    </div>    
    <div class="span4 offset2" id="lisbon">
      <h3><a href="https://www.facebook.com/events/315424058512362/">Lisbon, Portugal</a></h3>
      <dl class="dl-horizontal">
        <dt>Leandro Reis</dt>
        <dd><a href="https://twitter.com/Leandro_S_R">@Leandro_S_R</a></dd>
        <dd><a href="mailto:molyjamlisbon@gmail.com">Email the team</a></dd>
      </dl>
    </div>
  </div>
  <div class="row">
	<div class="span5 offset3" id="vegas">
      <h3><a href="https://www.facebook.com/events/355679211137575/">Las Vegas</a></h3>
      <dl class="dl-horizontal">
        <dt>Chris Ainsworth</dt>
        <dd><a href="https://twitter.com/driph">@driph</a></dd>
        <dd><a href="mailto:molyjamlv@gmail.com">Email the team</a></dd>
      </dl>
    </div>    
    <div class="span4 offset2" id="toronto">
      <h3><a href="https://www.facebook.com/events/392273507467116/">Toronto, Canada</a></h3>
      <dl class="dl-horizontal">
        <dt>Adam Proctor</dt>
        <dd><a href="https://twitter.com/ukengames">@ukengames</a></dd>
        <dd><a href="mailto:molyjamtoronto@gmail.com">Email the team</a></dd>
      </dl>
    </div>
  </div>
  <div class="row">
	<div class="span5 offset3" id="phoenix">
      <h3><a href="https://www.facebook.com/events/306886839379159/">Phoenix</a></h3>
      <dl class="dl-horizontal">
        <dt>Tyler Coleman</dt>
        <dd><a href="https://twitter.com/TylerMakesGames">@TylerMakesGames</a></dd>
        <dd><a href="mailto:molydeuxphoenix@gmail.com">Email the team</a></dd>
      </dl>
    </div>    
    <div class="span4 offset2" id="monterrey">
      <h3><a href="https://www.facebook.com/#!/events/319433411452017/">Monterrey, Mexico</a></h3>
      <dl class="dl-horizontal">
        <dt>Mike Olvera</dt>
        <dd><a href="https://twitter.com/Mike_alebrije">@Mike_alebrije</a></dd>
        <dd><a href="mailto:molyjammty@gmail.com">Email the team</a></dd>
      </dl>
    </div>
  </div>
  <div class="row">
	<div class="span5 offset3" id="austin">
      <h3><a href="https://www.facebook.com/events/396662183695103/">Austin</a></h3>
      <dl class="dl-horizontal">
        <dt>Amna Nadim</dt>
        <dd><a href="https://twitter.com/mochasips">@mochasips</a></dd>
        <dt>Osama Hashmi</dt>
        <dd><a href="https://twitter.com/BrewedThoughts">@BrewedThoughts</a></dd>
        <dt>Donald Harris</dt>
        <dd><a href="https://twitter.com/TallGamer">@TallGamer</a></dd>
        <dd><a href="mailto:molyjamatx@gmail.com">Email the team</a></dd>
      </dl>
    </div> 
    <div class="span4 offset2" id="tel aviv">
      <h3><a href="https://www.facebook.com/events/327359410657720/">Tel Aviv, Israel</a></h3>
      <dl class="dl-horizontal">
        <dt>Eli Brody</dt>
        <dd><a href="https://twitter.com/elibrody">@elibrody</a></dd>
        <dt>Yuval Sapir</dt>
        <dd><a href="https://twitter.com/yuvalsapir">@yuvalsapir</a></dd>  
        <dd><a href="mailto:molyjamta@gmail.com">Email the team</a></dd>                      
      </dl>
    </div>       
  </div>
  <div class="row">
	<div class="span5 offset3" id="lexington">
      <h3><a href="https://www.facebook.com/events/329487143774164/">Lexington</a></h3>
        <dl class="dl-horizontal">
	      <dt>John Meister</dt>
          <dd><a href="https://twitter.com/RunJumpDev">@RunJumpDev</a></dd>
          <dd><a href="mailto:molyjam@runjumpdev.com">Email the team</a></dd>
        </dl>
    </div>    
    <div class="span4 offset2" id="dundee">
      <h3><a href="https://www.facebook.com/events/370481596319539/">Dundee, Scotland</a></h3>
      <dl class="dl-horizontal">
         <dt>Paul McGee</dt>
         <dd><a href="https://twitter.com/paulmcgg">@PaulMcGG</a></dd>
         <dd><a href="mailto:molydundee@gmail.com">Email the team</a></dd>
      </dl>
    </div>    		
  </div>
  <div class="row">
	<div class="span5 offset3" id="malmo">
	  <h3><a href="https://www.facebook.com/events/252614434828906/">Malmo, Sweden</a></h3>
      <dl class="dl-horizontal">
        <dt>Erik Sjoblom</dt>
        <dd><a href="https://twitter.com/ErikSjoblom">@ErikSjoblom</a></dd>
        <dd><a href="mailto:molyjammalmo@gmail.com">Email the team</a></dd>
      </dl>
    </div>
    <div class="span4 offset2" id="christchurch">
	  <h3><a href="https://www.facebook.com/events/287156334686133/">Christchurch, New Zealand</a></h3>
      <dl class="dl-horizontal">
        <dt>James Tan</dt>        
        <dd><a href="mailto:molyjamnz@gmail.com">Email the team</a></dd>
      </dl>
    </div>	
  </div>
  <div class="row">
    <div class="span5 offset3" id="dieburg">  
      <h3><a href="http://www.facebook.com/events/415441525149226/">Dieburg, Germany</a></h3>
      <dl class="dl-horizontal">
        <dt>Oliver Eberlei</dt>
        <dd><a href="https://twitter.com/olivereberlei">@olivereberlei</a></dd>
        <dd><a href="mailto:molyjamdieburg@googlemail.com">Email the team</a></dd>
      </dl>    
    </div>		
    <div class="span4 offset2" id="helsinki">
      <h3><a href="https://www.facebook.com/events/287156334686133/">Helsinki, Finland</a></h3>
      <dl class="dl-horizontal">
        <dt>Petri Purho</dt>        
        <dd><a href="https://twitter.com/PetriPurho">@PetriPurho</a></dd>
        <dd><a href="molyjamhelsinki@gmail.com">Email the team</a></dd>
      </dl>
    </div>
  </div>
  <div class="row">
    <div class="span5 offset3" id="rio">  
      <h3><a href="http://www.facebook.com/events/275420929199400/">Rio de Janeiro, Brazil</a></h3>
      <dl class="dl-horizontal">
        <dt>Oliver Eberlei</dt>
        <dd><a href="https://twitter.com/clebertavares">@clebertavares</a></dd>        
      </dl>    
    </div>		
    <div class="span4 offset2"></div>
  </div>
<?php
    include_once('./templates/footer.php');
?>