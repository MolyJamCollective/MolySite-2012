...
class Check extends Generic {
	...
	private function isLoggedIn() {
		...
		if ( empty($_SESSION['username']) ) :
			...
			header('Location: '. SITE_PATH . 'login.php?e=1');
			exit();
		endif;
	...
	private function isRestricted() {
		...
		if ( !empty($_SESSION['restricted']) || !empty($_SESSION['level_disabled']) ) :
	...
	private function forcePwUpdate() {
		...
		if ( !empty($_SESSION['forcePwUpdate']) && basename($_SERVER['PHP_SELF']) != 'my-account.php') :
			header('Location: '. SITE_PATH . 'my-account.php?pe=1');
			exit();
		endif;
	...
	public function protectPage($level) {
		...
		*/
		...
		if( ! @array_intersect($level, $_SESSION['user_level']) && $level != array('*') )
			$this->deny_access();
	...
	private function deny_access() {
		...
		$error = "<div class='row'><div class='span12'>" . parent::getOption('block-msg') . "</div></div>";

		parent::displayErrors($error);
...
/** Helper functions for protecting pages */
